...
import { sessionRecord } from "../src/acp/session/setup.js";
...
describe("MCP over ACP (Issue #36)", () => {
  ...
  it("keeps MCP server credentials out of persisted session records", () => {
    const record = sessionRecord({
      ...
      selectedBaseModel: "test-model",
      selectedReasoningEffort: "medium",
      v2UserMessageIdsByStep: {}
    } as any);

    expect(record).not.toHaveProperty("mcpServers");
